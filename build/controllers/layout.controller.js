...
exports.createLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            ...
            ...(type === "Banner" && bannerImage ? { image: bannerImage } : {}),
        });
    }
    catch (error) {
...
exports.editLayout = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        if (type === "Banner") {
            ...
            const isCloudinaryUrl = hasImage && image.startsWith("https");
            const hasExistingImage = Boolean(bannerData?.banner?.image?.url && bannerData?.banner?.image?.url.trim());
            if (!hasImage && !hasExistingImage) {
                return next(new ErrorHandler_1.default("Please upload a banner image", 400));
            }
            let imagePayload = bannerData?.banner?.image || { public_id: "", url: "" };
            if (hasImage && !isCloudinaryUrl) {
                const uploaded = await cloudinary_1.default.v2.uploader.upload(image, {
                    folder: "layout",
                });
                imagePayload = {
            ...
            else if (hasImage && isCloudinaryUrl) {
                imagePayload = {
            ...
            await layout_model_1.default.findOneAndUpdate({ type: "Banner" }, { type: "Banner", banner }, { new: true, upsert: true, setDefaultsOnInsert: true });
