...
const normalizeAvatarForCloudinary = (avatar: string): string => {
  if (!avatar) return "";
  ...
  // Accept raw base64 payloads and default to PNG.
  return `data:image/png;base64,${trimmedAvatar}`;
};
...
export const updateProfilePicture = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const formattedAvatar = normalizeAvatarForCloudinary(avatar);
      ...
      if (avatar && user) {
        ...
        if (user?.avatar?.public_id) {
          ...
          const myCloud = await cloudinary.v2.uploader.upload(formattedAvatar, {
            folder: "avatars",
            width: 150,
          });
          user.avatar = {
